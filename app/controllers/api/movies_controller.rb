class API::MoviesController < API::ApplicationController
  ...
  skip_before_action :verify_authenticity_token, only: %i[destroy update]
  before_action :set_movie, only: %i[show destroy update]
  before_action :check_edit_permissions, only: %i[destroy update]
  ...
  def check_edit_permissions
    return unless @current_user.blank? || @current_user != @movie.user
