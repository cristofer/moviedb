...
module API
  module V2
    class Movies < Sinatra::Base
      ...
      patch "/", allows: [:movie, :params] do
        movie = Movie.find(params[:movie])
      
        unless movie.user == @user
          halt 500, {error: "You are not authorized to update the movie"}.to_json 
        end
      ...
      delete "/", allows: [:movie] do
        movie = Movie.find(params[:movie])

        unless movie.user == @user
          halt 500, {error: "You are not authorized to delete the movie"}.to_json
        end
